...
const (
	StreamingService_BidirectionalStreaming_FullMethodName = "/StreamingService.StreamingService/BidirectionalStreaming"
)
...
type StreamingServiceClient interface {
	BidirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamRequest, StreamResponse], error)
}
...
func (c *streamingServiceClient) BidirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamRequest, StreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &StreamingService_ServiceDesc.Streams[0], StreamingService_BidirectionalStreaming_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type StreamingService_BidirectionalStreamingClient = grpc.BidiStreamingClient[StreamRequest, StreamResponse]
...
type StreamingServiceServer interface {
	BidirectionalStreaming(grpc.BidiStreamingServer[StreamRequest, StreamResponse]) error
...
func (UnimplementedStreamingServiceServer) BidirectionalStreaming(grpc.BidiStreamingServer[StreamRequest, StreamResponse]) error {
...
func _StreamingService_BidirectionalStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(StreamingServiceServer).BidirectionalStreaming(&grpc.GenericServerStream[StreamRequest, StreamResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type StreamingService_BidirectionalStreamingServer = grpc.BidiStreamingServer[StreamRequest, StreamResponse]
